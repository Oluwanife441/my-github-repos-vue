...
export default {
  ...
  methods: {
    fetchData: function () {
      this.loading = true
      fetch(`https://api.github.com/repos/Oluwanife441/${this.$route.params.id}`, {
        headers: {
      ...
      fetch(`https://api.github.com/repos/Oluwanife441/${this.$route.params.id}/branches`, {
        headers: {
          Accept: 'application/json'
        }
      })
      ...
      fetch(`https://api.github.com/repos/Oluwanife441/${this.$route.params.id}/deployments`, {
        headers: {
          Accept: 'application/json'
        }
      })
...
<template>
  <div id="repodetail">
    <div class="back-wrapper">
      <router-link :to="`/`">
        <button class="back"><BackIcon /> back</button>
      </router-link>
    </div>
    ...
    <div v-else class="repodetail-card">
      ...
      <p v-else>
        Live site:
        <a :href="`https://oluwanifemi.github.io/${details.name}`"
          >Oluwanife441.github.io/{{ details.name }}</a
        >
      </p>
